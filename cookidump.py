...
def startBrowser(chrome_driver_path):
    ...
    # As there is no need anymore to specify a direct path just getting rid of it
...
def recipeToJSON(browser, recipeID):
    ...
    recipe["rating_count"] = re.sub(
        r"\D", "", soup.select_one(".core-rating__label").text, flags=re.IGNORECASE
    )
    recipe["rating_score"] = soup.select_one(".core-rating__counter").text
    recipe["tm-versions"] = [
        v.text.replace("\n", "").strip().lower()
        for v in soup.select(".recipe-card__tm-version core-badge")
    ]
    recipe.update(
        {
            l.text: l.next_sibling.strip()
            for l in soup.select("core-feature-icons label span")
        }
    )
    recipe["ingredients"] = [
        re.sub(" +", " ", li.text).replace("\n", "").strip()
        for li in soup.select("#ingredients li")
    ]
    recipe["nutritions"] = {}
    for item in list(
        zip(
            soup.select(".nutritions dl")[0].find_all("dt"),
            soup.select(".nutritions dl")[0].find_all("dd"),
        )
    ):
        dt, dl = item
        recipe["nutritions"].update(
            {
                dt.string.replace("\n", "")
                .strip()
                .lower(): re.sub(
                    r"\s{2,}", " ", dl.string.replace("\n", "").strip().lower()
                )
            }
        )
    recipe["steps"] = [
        re.sub(" +", " ", li.text).replace("\n", "").strip()
        for li in soup.select("#preparation-steps li")
    ]
    recipe["tags"] = [
    ...
    hints = [
        hint.get_text(separator=" ").strip()
        for hint in soup.select("#hints-and-tricks ul li")
    ]
    if hints:
        recipe["hints"] = hints
...
def run(outputdir, separate_json, webdriverfile=None, country=None):
    """Scraps all recipes and stores them in html"""
    print("[CD] Welcome to cookidump, starting things off...")

    # fixing the outputdir parameter, if needed, shoud be cross-platform compatible
    ...
    if country is None:
        ...
        lang = str(input("[CD] Complete the website domain: https://cookidoo."))

    baseURL = "https://cookidoo.{}/".format(lang)
    ...
    # recipes base url
    rbURL = "https://cookidoo.{}/search/".format(lang)
    brw.get(rbURL)
    time.sleep(PAGELOAD_TO)
    ...
    custom_output_dir = input(
        "[CD] enter the directory name to store the results (ex. vegeratian): "
    )
    if custom_output_dir:
        outputdir += "{}/".format(custom_output_dir)
    # proceeding
    print("[CD] Proceeding with scraping")
    # removing the name
    brw.execute_script(
        "var element = arguments[0];element.parentNode.removeChild(element);",
        brw.find_element(By.TAG_NAME, "core-user-profile"),
    )
    # clicking on cookie accept
    try:
        brw.find_element(By.CLASS_NAME, "accept-cookie-container").click()
    except:
        pass
    # showing all recipes
    elementsToBeFound = int(
        brw.find_element(By.CLASS_NAME, "items-start")
        .text.split("\n")[-1]
        .split(" ")[0]
    )
    previousElements = 0
    ...
    try:
        brw.execute_script(
            "var element = arguments[0];element.parentNode.removeChild(element);",
            brw.find_element(By.TAG_NAME, "core-search-bar"),
        )
    except:
        pass
    ...
    for recipeURL in recipesURLs:
        try:
            ...
            try:
                brw.execute_script(
                    "var element = arguments[0];element.parentNode.removeChild(element);",
                    brw.find_element(By.TAG_NAME, "base"),
                )
            except:
                pass
            # removing the name
            ...
            brw.execute_script(
                "arguments[0].setAttribute(arguments[1], arguments[2]);",
                brw.find_element(By.CLASS_NAME, "page-header__home"),
                "href",
                "../../index.html",
            )
            # saving recipe image
            img_url = (
                brw.find_element(By.ID, "recipe-card__image-loader")
            ...
            brw.execute_script(
                "arguments[0].setAttribute(arguments[1], arguments[2]);",
                brw.find_element(By.CLASS_NAME, "core-tile__image"),
                "srcset",
                "",
            )
            brw.execute_script(
            ...
            if separate_json:
                ...
                with open(
                    "{}recipes/{}.json".format(outputdir, recipeID), "w"
                ) as outfile:
            ...
            c += 1
            if c % 10 == 0:
                print("Dumped recipes: {}/{}".format(c, len(recipesURLs)))
        except:
            pass
    ...
    if not separate_json:
        ...
        with open("{}data.json".format(outputdir), "w") as outfile:
            json.dump(recipeData, outfile)

    # logging out
    logoutURL = "https://cookidoo.{}/profile/logout".format(lang)
    brw.get(logoutURL)
    time.sleep(PAGELOAD_TO)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Dump Cookidoo recipes from a valid account"
    )

    debugging = True
    # At the moment webdriver is not needed anymore to set
    if debugging:
        print("[CD] Running under a debugger, using fixed values.")
        outputdir = outputdir
        separate_json = True
    else:
        ...
        parser.add_argument(
            ...
            "--separate-json",
            action="store_true",
            help="Create a separate JSON file for each recipe; otherwise, a single data file will be generated",
        )
        args = parser.parse_args()
        outputdir = args.outputdir
        separate_json = args.separate_json

    # driver can be directly used since chrome 115
    # driver = webdriver.Chrome()
    run(outputdir, separate_json)
